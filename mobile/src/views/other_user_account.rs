...
pub fn OtherUserAccount(user_id: u64, credentials: AccountCredentials) -> Element {
    let user_data = future_retry_loop!(get_user_data(user_id, credentials));
                ...
                result = rsx! {
                    ...
                    button {
                        ...
                        onclick: move |_| {
                            let user_data = user_data.clone();
                            async move {
                                match send_group_invite(user_id, group.id, GroupPermissions::default().to_bytes(), credentials, generate_encrypted_shared_key(group.id, user_data.clone(), false)).await {
                                    Ok(invite_id) => {
            ...
            if matches!(user_data, PacketState::Response(_)) {
                ...
                button {
                    onclick: move |_| {
                        ...
                        async move {
                            match send_dm_invite(user_id, generate_encrypted_shared_key(user_id, user_data.clone(), true), credentials).await {
                                Ok(invite_id) => {
