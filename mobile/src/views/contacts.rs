...
pub fn Contacts(credentials: AccountCredentials) -> Element {
    ...
    let joined_dm_groups = future_retry_loop!(get_joined_dm_groups(credentials));
    let joined_groups = future_retry_loop!(get_joined_groups(credentials));
                ...
                input {
                    ...
                    oninput: move |event| async move {
                        ...
                        if query.is_empty() {
                            ...
                            match find_user(query, credentials).await {
...
fn DmMessagesPanel(selected_dm_group: DmGroup, credentials: AccountCredentials) -> Element {
    ...
    future_retry_loop! { dm_messages_signal, dm_messages_resource, fetch_new_dm_messages(selected_dm_group.id, 0, credentials) };
    use_effect(move || {
            ...
            div {
                ...
                textarea {
                    ...
                    onkeydown: move |event| async move {
                        ...
                        println!("Send result: {:?}", send_dm_message(
                        ...
                        //     .retry_loop(move || send_dm_message(
                        //         selected_dm_group.id,
                        //         "plain".to_owned(),
                        //         msg_bytes.clone(),
                        //         credentials,
...
fn GroupMessagesPanel(selected_group: MultiUserGroup, credentials: AccountCredentials) -> Element {
    ...
    future_retry_loop! { group_messages_signal, group_messages_resource, fetch_new_group_messages(selected_group.id, 0, credentials) };
    use_effect(move || {
            ...
            div {
                ...
                textarea {
                    ...
                    onkeydown: move |event| async move {
                        ...
                        println!("Send result: {:?}", send_group_message(
                            selected_group.id,
                            encryption_method,
                            msg_bytes,
                            credentials,
...
pub fn GroupPanel(
    ...
    let members_data = future_retry_loop!(get_group_member_count(group.id, credentials));
...
fn GroupMessageComponent(
    ...
    let message_content = if message.encryption_method != "plain" {
        ...
        rsx!({String::from_utf8_lossy(message.content.as_ref().unwrap())})
    };
    rsx! {
        {author}
        div {
